...
export function generateHeuristicBriefing(
  ...
  const keyDrivers = [
    {
      ...
      description: metrics.totalComplaints > 0 
        ? `Основний масив скарг за період ${dateLabel} стосувався швидкості завантаження даних та стабільності сигналу під час пересування (${topLoc}).`
  ...
  const churnRiskAnalysis = metrics.churnIntentCount > 0
    ...
    : `🟢 Ризик відтоку мінімальний: погроз відмови від послуг або переходу до інших операторів за звітний період не зафіксовано. Лояльність клієнтської бази залишається на високому рівні.`;
...
Твоє завдання — скласти офіційний ранковий аналітичний бриф (Morning Briefing) для топ-менеджменту компанії за 7 днів (${dateLabel}).
