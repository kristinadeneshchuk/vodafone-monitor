...
def collect_google_news(brand_query, display_brand):
    mentions = []
    rss_url = f"https://news.google.com/rss/search?q={display_brand}&hl=uk&gl=UA&ceid=UA:uk"
...
def run_collectors():
    ...
    for brand, app_id in PLAY_STORE_APPS.items():
        all_mentions.extend(collect_play_reviews(brand, app_id))
        
    brands = {'vodafone': 'Vodafone', 'kyivstar': 'Київстар', 'lifecell': 'lifecell'}
